...
#' into 2 groups, plots Kaplan-Meier survival curves for these groups,
#' and observed vs. expected optimization plot. Does not use the permutation
#' test to estimate the statistical significance of the optimal cutoff.
#'
...
#' where samples are in rows. First column must contain sample ids that
#' match those in 'fname'. The file must contain columns
#' called 'stime' and 'scens',
...
#' adding "_KMopt_minf_.2f" and corresponding extension to 'fname'.
...
#'
#' kmoptscut(fname = fdat, sfname = sdat, wpdf = FALSE)
#'
