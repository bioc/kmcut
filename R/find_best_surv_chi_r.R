# This function returns the best Chi-square from K-M minimization based on
# pracma findpeaks()
# and Spearman correlation between the observed and ideal linear plot Chi-sq
# vs cutoffs
# Returns a vector (best_cutoff, chi_sq, corr)
# Variables 'stime' and 'scens' are vectors with the survival times and censor
...
# 'minfrac' - the value of the smallest allowed group size for K-M
# partitioning (in [0, 1])
# 'min_up_dn' - the number of up/down steps before/after peak for findpeaks
#' @keywords internal
find_best_surv_chi_r <- function(mc.stime, mc.scens, mc.featdata,
                                    mc.ideal_y, mc.minfrac = 0.1,
                                    mc.min_up_dn = 1, mc.tolerance = 0.1)
{
    if(length(mc.stime) != length(mc.scens) || length(mc.stime) !=
        length(mc.featdata) || length(mc.scens) != length(mc.featdata))
    {
    ...
    mc.sur <- survdiff(Surv(time=mc.stime, event=mc.scens,
                            type = 'right') ~ mc.fact, rho = 0)
    mc.chi_values <- c(mc.chi_values,mc.sur$chisq)
    mc.cutoffs <- c(mc.cutoffs,mc.cutoff)
    }

    # Peak finder
    mc.peaks <- findpeaks(mc.chi_values, nups = mc.min_up_dn,
                        ndowns = mc.min_up_dn, npeaks=5, sortstr=TRUE,
    ...
    # If there are peaks with nearly equal height,
    # select the one that results in the most balanced (closest
    # to the median) split
    # mc.tolerance <- .Machine$double.eps ^ 0.5
    mc.best_peaks_i <-
    mc.peaks[abs(mc.peaks[,1] - mc.peaks[1,1]) < mc.tolerance, 2]
    ...
    # The ideal plot is defined by two lines with intercepts
    # and slopes alpha1,alpha2 and beta1,beta2
    mc.r <- cor(mc.chi_values, mc.ideal_y, method = "spearman",
        use ="na.or.complete")
